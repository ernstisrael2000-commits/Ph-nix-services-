...
export default function UserAuthModal({ open, onOpenChange, onClientLogin, onAdminLogin, onAffiliateAccess }: UserAuthModalProps) {
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="max-w-sm rounded-[2rem] p-0 overflow-hidden border-0 shadow-2xl" showCloseButton={false}>
        <AnimatePresence mode="wait">
          ...
          {/* ── ADMIN ACCESS ── */}
          {view === 'admin-access' && (
            <motion.div key="admin-access" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
              ...
              <div className="p-6 space-y-4 bg-white">
                <div className="space-y-2">
                  ...
                  <button
                    ...
                    disabled={loading}
                    className="w-full flex items-center justify-center gap-3 h-14 rounded-2xl border-2 border-gray-200 hover:border-blue-300 hover:bg-blue-50/30 transition-all active:scale-95 disabled:opacity-60 disabled:cursor-not-allowed group bg-white shadow-sm"
                  >
                    {loading ? (
                ...
                <div className="space-y-2">
                  ...
                  <button
                    onClick={() => { onAffiliateAccess(); handleClose(false); }}
                    className="w-full flex items-center gap-4 p-4 rounded-2xl border-2 border-gray-100 hover:border-amber-300 hover:bg-amber-50/30 transition-all group text-left active:scale-95"
                  >
                    ...
                    <div>
                      ...
                      <p className="text-xs text-subtext">Tableau de bord affiliés et agents</p>
                    </div>
                  </button>
                </div>
              </div>
