...
export default function SafacilPaySuccessView({ referenceId, transactionId, onClose }: Props) {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        const res  = await fetch(`/api/payments/safacilpay/status/${referenceId}`);
        const data = await res.json();
        if (data.status === 'completed') {
          setUsd(data.usdAmount); setHtg(data.htgAmount); setStatus('completed');
        } else if (data.status === 'failed' || data.status === 'cancelled') {
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm px-4">
      ...
      >
        <div className={`p-6 bg-gradient-to-br ${gradientCls} text-white text-center`}>
          <p className="text-[10px] font-black uppercase tracking-widest opacity-70 mb-3">SafacilPay</p>
            ...
            {isLoading && (
              <motion.div key="loading" initial={{ scale: 0 }} animate={{ scale: 1 }} className="flex flex-col items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-white/70 text-sm mt-1">En attente de confirmation SafacilPay</p>
                </div>
              </motion.div>
            )}
            {isTimeout && (
              <motion.div key="timeout" initial={{ scale: 0 }} animate={{ scale: 1 }} className="flex flex-col items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-white/70 text-sm mt-1">Vérification en cours côté SafacilPay</p>
                </div>
              </motion.div>
            )}
            {isSuccess && (
          ...
          {(isLoading || isTimeout) && (
            <div className="bg-blue-50 border border-blue-100 rounded-2xl p-4 text-center">
              <p className="text-[11px] text-blue-800 font-bold leading-relaxed">
                Votre solde sera mis à jour automatiquement dès confirmation par SafacilPay.
              </p>
