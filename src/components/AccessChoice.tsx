...
interface AccessChoiceProps {
  onChoice: (choice: 'affiliate' | 'admin' | 'agent') => void;
}
      ...
      <div className="max-w-6xl w-full grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        >
          <Card 
            ...
            className="group relative h-full overflow-hidden border-0 shadow-xl rounded-[2.5rem] cursor-pointer hover:shadow-2xl transition-all duration-500 hover:-translate-y-2 bg-white ring-2 ring-emerald-50"
          >
            <div className="absolute top-0 left-0 w-full h-2 bg-emerald-500" />
            <div className="p-8 sm:p-10 flex flex-col items-center text-center">
              <div className="w-20 h-20 rounded-3xl bg-emerald-100 flex items-center justify-center mb-6 group-hover:scale-110 transition-transform duration-500">
                <UserCheck className="h-10 w-10 text-emerald-600" />
              </div>
              <h3 className="text-2xl font-black text-dark mb-3">Espace Agent</h3>
              <p className="text-gray-500 font-medium mb-8">
                Agents de dépôt physique : Validez les transactions et gérez votre solde.
              </p>
              <div className="flex items-center gap-2 text-emerald-600 font-black uppercase text-sm tracking-widest group-hover:gap-4 transition-all mt-auto">
                Accès Agent <ArrowRight className="h-4 w-4" />
              </div>
            </div>
